...
> **Security note**: `docker-compose.yml` ships with hard-coded demo
> credentials (Postgres, MinIO, Keycloak admin, OpenBao dev root token,
> Grafana) so the stack boots without any setup. These are fine for local
> development and the SIH demo, but every one of them **must** be changed
> — and OpenBao in particular must be switched from `server -dev` to a
> properly initialized/unsealed production mode — before this is exposed
> anywhere other than localhost. `CORS_ALLOWED_ORIGINS` in `.env` also
> defaults to no cross-origin access; set it explicitly to your frontend's
> real origin(s) rather than `*` in any real deployment.
...
>>>>>>> 7c12f3d (Prototype V1 integrated all the services requires testing and model integration)
