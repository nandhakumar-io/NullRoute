...
from app.services.config_merge import config_hash as _canonical_config_hash
...
class CollectionResult:
    ...
    def __post_init__(self):
        if self.success and self.raw_config and not self.config_hash:
            # Canonical (volatile-line-stripped) hash -- see
            # services/config_merge.py::canonical_lines/config_hash. A raw
            # sha256 of the exact bytes made every pre-deployment
            # "has the device changed since validation?" check and every
            # post-deployment "did the push actually apply?" check brittle
            # against vendor-inserted volatile lines (timestamp banners like
            # "! Last configuration change at 10:15:32 UTC", NVRAM
            # metadata, `## Last commit: ...`, etc.) that change on every
            # collection even when nothing meaningful did -- causing false
            # ABORTED_STALE_HASH aborts and false DRIFTED verdicts. This
            # affects every consumer of CollectionResult.config_hash
            # (deployment_service.py's pre/post checks, rollback_service.py's
            # post-rollback verification), all of which want "did the
            # functional configuration change", not "are the exact bytes
            # identical".
            self.config_hash = _canonical_config_hash(self.raw_config)
