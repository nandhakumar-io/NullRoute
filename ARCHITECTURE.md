...
FastAPI (Pydantic, SQLAlchemy)  ←─→  Keycloak (OIDC/RBAC)
 ...
 ┌─────┼────────────────────────┬──────────────────┐
 ▼     ▼                        ▼                  ▼
CLI   Deterministic Parsers   Local AI (Ollama)  OPA / Rego Engine
      (Regex/AST driven)      (Qwen3-8B + RAG)
 └──────────────────────────────┼──────────────────┘
                                ▼
...
## 3. Decentralized and Immutable Evidence Anchoring
...
- The `fabric-gateway` acts as a bridging layer to the Hyperledger Fabric chain (built on Go chaincodes).
- At verification time, `/api/evidence/{id}/verify` cryptographically recomputes the SHA-256 hash array of the stored JSON record, comparing it symmetrically against the immutable `evidenceHash` situated on-chain. If any database manipulation is detected out-of-bounds of the Fabric log, it registers as a fundamental `INTEGRITY_FAILURE` to the pipeline monitor.

## 4. Robust Asynchronous Pipeline Lifecycle
