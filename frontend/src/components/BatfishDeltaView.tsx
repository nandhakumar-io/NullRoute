...
function NodeDeltaPanel({ delta }: { delta: NonNullable<SnapshotDiff["node_delta"]> }) {
  if (delta.added.length === 0 && delta.removed.length === 0) {
    return <div className="text-sm text-slate-500">No topology node changes.</div>;
  }
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <div className="text-sm uppercase tracking-wide text-emerald-500 font-semibold mb-1">
        ...
        {delta.added.length === 0 ? (
          ...
          <ul className="space-y-0.5">
            {delta.added.map((n) => (
              <li key={n} className="text-sm font-mono text-emerald-300">+ {n}</li>
            ))}
          </ul>
        )}
      </div>
      <div>
        <div className="text-sm uppercase tracking-wide text-red-500 font-semibold mb-1">
        ...
        {delta.removed.length === 0 ? (
          ...
          <ul className="space-y-0.5">
            {delta.removed.map((n) => (
              <li key={n} className="text-sm font-mono text-red-300">− {n}</li>
            ))}
          </ul>
        )}
      </div>
...
function RouteDeltaPanel({ delta }: { delta: NonNullable<SnapshotDiff["route_delta"]> }) {
  ...
  return (
    <div className="flex items-center gap-6 text-base">
      <div>
        <div className="text-sm text-slate-500">Current routes</div>
        <div className="font-mono text-slate-200">{delta.current_count}</div>
      </div>
      <div className="text-slate-600">→</div>
      <div>
        <div className="text-sm text-slate-500">Proposed routes</div>
...
function FlowDiffRow({ f }: { f: BatfishFlowDiff }) {
  ...
  return (
    <tr className={f.changed ? "bg-amber-950/10" : undefined}>
      <td className="px-3 py-2 text-sm text-slate-300">{f.title}</td>
      <td className="px-3 py-2 text-sm font-mono text-slate-400">{f.source_zone} → {f.destination_zone}</td>
      ...
      <td className={`px-3 py-2 text-sm font-semibold ${resultTone}`}>{f.result}</td>
    </tr>
  );
}
...
export default function BatfishDeltaView({ diff }: { diff: SnapshotDiff }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {diff.node_delta && (
        <div className="card">
          <div className="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-3">Topology Changes</div>
      ...
      {diff.route_delta && (
        <div className="card">
          <div className="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-3">Route Table Impact</div>
      ...
      {diff.differential_reachability && (
        <div className="card">
          <div className="text-sm uppercase tracking-wide text-slate-500 font-semibold mb-2">Differential Reachability</div>
      ...
      {diff.flow_diffs && diff.flow_diffs.length > 0 && (
        <div className="overflow-hidden rounded-lg border border-soc-border">
          <table className="w-full text-left text-base text-slate-400">
            <thead className="bg-soc-panel border-b border-soc-border uppercase text-sm">
              <tr>
      ...
      {!diff.node_delta && !diff.route_delta && !diff.differential_reachability && (!diff.flow_diffs || diff.flow_diffs.length === 0) && (
        <div className="text-sm text-slate-500">Batfish diff completed with no reportable deltas.</div>
      )}
    </div>
  );
}